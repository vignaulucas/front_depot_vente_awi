...
const NonUserParticularFinancialSummary: React.FC = () => {
    ...
    useEffect(() => {
        if (selectedSession && selectedSeller) {
            fetch(`${apiUrl}/transactions/summary/${selectedSession.id}/${selectedSeller.id}`, {
                headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
            })
                .then(response => {
                    if (!response.ok) {
    ...
    useEffect(() => {
        if (selectedSession && selectedSeller) {
            fetch(`${apiUrl}/depot/sellGamesNonUser/${selectedSession.id}/${selectedSeller.id}`, {
                headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
            })
                .then(response => {
                    if (!response.ok) {
...
export default NonUserParticularFinancialSummary;
